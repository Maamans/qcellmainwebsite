


export default function QCellAboutSection() {
  return (
    <section className="py-24 px-4 md:px-8 bg-white w-full flex items-center justify-center">
      <div className="max-w-5xl w-full flex flex-col items-center justify-center text-center mx-auto">
        <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold text-gray-900 mb-12 tracking-tight leading-tight">About QCell</h2>
        <div className="space-y-8 text-gray-700 max-w-4xl w-full mx-auto">
          <p className="text-lg md:text-xl lg:text-2xl leading-relaxed font-light text-left">
            QCell is Sierra Leone&#39;s only African-owned telecom company, pioneering mobile and internet services nationwide.
          </p>
          <p className="text-lg md:text-xl leading-relaxed text-left">
            QCell is the very first Mobile company to conduct a soft launch by offering services not only in the capital city Freetown but also to all provincial district headquarter towns with 3G mobile internet service as well as enhanced voice and SMS services.
          </p>
          <p className="text-lg md:text-xl leading-relaxed text-left">
            QCell has coverage presence in all 16 districts in the country today offering 3G/4G services across the country. 5G roll out is on the cards as few deployment sites are currently been on trial.
          </p>
        </div>

        <div className="mt-16 pt-16 border-t border-gray-200 max-w-4xl w-full mx-auto">
          <h4 className="text-3xl md:text-4xl font-bold text-[#F98F1F] mb-6 text-left">Salone Pride</h4>
          <p className="text-lg md:text-xl text-gray-700 leading-relaxed text-left">
            Salone pride is the QCell philosophy meaning &#39;ours&#39; in Krio language. We believe that QCell is of Sierra Leoneans, for Sierra Leoneans and by Sierra Leoneans. Salone Pride is Sierra Leoneans uniting our nation and supporting each other&#39;s growth.
          </p>
        </div>
      </div>
    </section>
  )
}
