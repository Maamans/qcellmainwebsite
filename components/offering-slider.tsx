...
export default function OfferingsSlider({ offerings: offeringsProp }: OfferingsSliderProps = {}) {
  ...
  const [offeringsData, setOfferingsData] = useState<Offering[]>(
    Array.isArray(offeringsProp) && offeringsProp.length > 0 ? offeringsProp : fallbackOfferings,
  )
  ...
  useEffect(() => {
    if (Array.isArray(offeringsProp) && offeringsProp.length > 0) {
      setOfferingsData(offeringsProp)
    }
  }, [offeringsProp])
  ...
  useEffect(() => {
    ...
    const loadOfferings = async () => {
      try {
        ...
        if (isMounted) {
          setOfferingsData(mergeOfferings(normalized, fallbackOfferings))
        }
      } catch (error) {
        console.error("Failed to load offerings:", error)
        if (isMounted) {
          setOfferingsData(fallbackOfferings)
        }
      } finally {
      }
    }
